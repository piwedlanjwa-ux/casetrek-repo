<?php
    ...
    $status_err = $date_of_dismissal_err = "";
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        //Validate status
        if($_POST["status"] == "Select Status" || $_POST["status"] == "Open")
        {
        ...
        //Validate date of dismissal
        if($_POST["sanction"] != "Dismissal")
        {
            $date_of_dismissal = "";
        }
        elseif($_POST["sanction"] == "Dismissal")
        {
            $date_of_dismissal = $_POST["date_of_dismissal"];
        }
        else
        {
            $date_of_dismissal_err = "Please pick date of dismissal";
        }
        ...
        while($that = mysqli_fetch_array($result))
        {
            if($that['notes_file_name'] == $fileName)
            {
                $fileExistsFlag = 1;
            }
        }
        ...
        if($fileExistsFlag == 0)
        {
            ...
            if($result)
            {
                if(empty($status_err))
                {
                    ...
                    $sql = "INSERT INTO closed_casestb (case_id, sanction, validation, status, notes_file_name, date_of_dismissal) VALUES (?, ?, ?, ?, ?, ?)";
                    
                    if($stmt =  mysqli_prepare($DBConnect, $sql))
                    {                        
                        // Bind variables to the prepared statement as parameters
                        mysqli_stmt_bind_param($stmt, "ssssss", $param_case_id, $param_sanction, $param_validation, $param_status,  $param_notes_file_name, $param_date_of_dismissal);
        ...
        {
            echo "File <html><b><i>".$fileName."</i><b></html> already exists in your notes.";
        }
            ...
            <div id="content">
                ...
                <div class="login-box">
                        <div class="login-box-body">
                            <h4 class="login-box-msg">Close Case No. <?php echo $id?></h4>
                            <form>
                ...
                <div class="login-box">
                    <div class="login-box-body">     
                        <form action="CloseCase.php?id=<?php echo $id; ?>" method="POST" enctype="multipart/form-data">
                            <div class="form-group row">
                                <div class="col-sm-3"><label>Sanction</label></div>
                                <div class="col-sm-7">
                                <select class="form-control" type="text" name="sanction" value="<?php echo $that['sanction']; ?>">
                            ...
                            <div class="form-group row <?php echo (!empty($status_err)) ? 'has-error' : ''; ?>">
                                <div class="col-sm-3"><label>Status</label></div>
                                <div class="col-sm-7">
                                    <select class="form-control" name="status" value="<?php print $that['status']; ?>">
                            ...
                            <div class="form-group row <?php echo (!empty($date_of_dismissal_err)) ? 'has-error' : ''; ?>">
                                <div class="col-sm-3"><label>Date of Dismissal</label></div>
                                    <div class="col-sm-7"><input class="form-control" type="date" name="date_of_dismissal" value="<?php echo $date_of_dismissal; ?>" />
                                </div>
                                <span class="help-block"><?php echo $date_of_dismissal_err; ?></span>
                            </div>
                            ...
                            <input type="submit" name="close" class="btn btn-primary" value="Close"/>&nbsp;<input type="reset" class="btn btn-outline-primary" value="Clear"/>&nbsp;<a href="ViewCase.php?id=<?php print $id; ?>"><input type="cancel" class="btn btn-outline-secondary" value="Cancel"/></a>
                        </form>
