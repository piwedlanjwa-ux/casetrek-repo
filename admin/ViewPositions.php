    ...
    if (isset($_POST["import"])) {
        ...
        if ($_FILES["file"]["size"] > 0) {
            ...
            while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
                ...
                $position= "";
                if (isset($column[0])) {
                    $position = mysqli_real_escape_string($connect, $column[0]);
                } 

                $department= "";
                if (isset($column[1])) {
                    $department = mysqli_real_escape_string($connect, $column[1]);
                } 
                
                $sqlInsert = "INSERT into positions (position, department)
                       values (?, ?)";
                $paramType = "ss";
                $paramArray = array(
                    $position,
                ...
                <div class='login-box'>
                    <div class='login-box-body'>
                        <div class="table-title">
                            <div class="row">
                                <div class="col-sm-8"><h3><b>Job Titles</b></h3></div>
                                <div class="col-sm-4">
                                    <a href="AddPosition.php"><button type="button" class="btn btn-primary add-new"><i class="fa fa-plus"></i> New Job Title</button></a>
                                </div>
                            </div>
                            <br>
                            <div class="row">
                        ...
                        <?php
                            ...
                            if($QueryResult)
                            {
                                if($QueryResult->num_rows>0)
                                {
                                    ...
                                    echo "<tr><th></th><th>Job Title</><th>Department</th><th>Action</th></tr>";
                                    while($row = $QueryResult->fetch_assoc())
                                    {
                                        echo "<tr><td></td>";
                                        echo "<td>". $row['position'] . "</td>";
                                        echo "<td>". $row['department'] . "</td>";
                                        echo "<td><a href ='EditPosition.php?id= " . $row['pos_id'] . "'>Edit</a> | <a href ='DeletePosition.php?id= " . $row['pos_id'] . "'>Delete</a></td></tr>";
                                    }
                                    echo "</table>";
