    ...
    if (isset($_POST["import"])) {
        ...
        if ($_FILES["file"]["size"] > 0) {
            ...
            while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
                ...
                $email = "";
                if (isset($column[2])) {
                    $email = mysqli_real_escape_string($connect, $column[2]);
                }
                $password = "";
                if (isset($column[3])) {
                    $password = mysqli_real_escape_string($connect, $column[3]);
                }
                $userType = "";
                if (isset($column[4])) {
                    $userType = mysqli_real_escape_string($connect, $column[4]);
                }
                $jobTitle = "";
                if (isset($column[5])) {
                    $jobTitle = mysqli_real_escape_string($connect, $column[5]);
                }
                ...
                $sqlInsert = "INSERT into userstbl (firstName, lastName, email, password, userType, jobTitle)
                       values (?,?,?,?,?,?)";
                $paramType = "ssssss";
                $paramArray = array(
                    $firstName,
...
<html>
    ...
    <body>
                ...
                <div class='login-box'>
                    <div class='login-box-body'>
                        <div class="table-title">
                            <div class="row">
                                <div class="col-sm-8"><h3><b>Users</b></h3></div>
                                <div class="col-sm-4">
                                    <a href="AddUser.php"><button type="button" class="btn btn-primary add-new"><i class="fa fa-plus"></i> New User</button></a>
                                </div>
                            </div>
                            <br>
                            <div class="row">
        ...
        <?php
            ...
            $SQLString = "SELECT * FROM `userstbl`";
            ...
            if($QueryResult)
            {
                if($QueryResult->num_rows>0)
                {
                    ...
                    echo "<tr><th></th><th>First Name</th><th>Last Name</th><th>Email Address</th><th>User Type</th><th>Job Title</th><th>Action</th></tr>";
                    while($row = $QueryResult->fetch_assoc())
                    {
                        ...
                        echo "<td><a href ='EditUser.php?id= " . $row['user_id'] . "'>Edit</a> | <a href ='DeleteUser.php?id= " . $row['user_id'] . "'>Delete</a></td></tr>";
                    }
                    echo "</table>";
