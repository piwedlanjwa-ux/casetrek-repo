<?php
    ...
    $email_err =$firstName_err = $lastName_err = $department_err = $jobTitle_err = "";
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        else
        {
            ...
            $sql = "SELECT emp_id, firstName, lastName FROM employeestbl WHERE email = ?";
            ...
            if($stmt = mysqli_prepare($DBConnect, $sql)){
                ...
                if(mysqli_stmt_execute($stmt)){
                    ...
                    if(mysqli_stmt_num_rows($stmt) == 1){
                        $email_err = "Employee with this email address already exists.";
                    } else{
        ...
        if(empty($firstName_err) && empty($lastName_err) && empty($email_err) && empty($department_err) && empty($jobTitle_err)){
            
            // Prepare an insert statement
            $sql = "INSERT INTO employeestbl (firstName, lastName, jobTitle, department, email ) VALUES (?, ?, ?, ?, ?)";
            
            if($stmt = mysqli_prepare($DBConnect, $sql)){
                // Bind variables to the prepared statement as parameters
                mysqli_stmt_bind_param($stmt, "sssss", $param_firstName, $param_lastName, $param_jobTitle, $param_department, $param_email);
            ...
            <div id="content">
                ...
                <div class="login-box">
                    <div class="login-box-body">
                        <h1 class="login-box-msg">Add Employee</h1>

                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">

                            <div class="form-group row <?php echo (!empty($firstName_err)) ? 'has-error' : ''; ?>">
                                <div class="col-sm-3"><label>Name</label></div>
                                <div class="col-sm-7"><input type="text" class="form-control" id="firstName" name="firstName" placeholder="First name" value="<?php echo $firstName; ?>"/></div>
                                <span class="help-block"><?php echo $firstName_err; ?></span>
                            </div>
                            <div class="form-group row <?php echo (!empty($lastName_err)) ? 'has-error' : ''; ?>">
                                <div class="col-sm-3"><label>Surname</label></div>
                                <div class="col-sm-7"><input type="text" class="form-control" id="lastName" name="lastName" placeholder="Last name" value="<?php echo $lastName; ?>"/></div>
                                <span class="help-block"><?php echo $lastName_err; ?></span>
                            </div>
                            <div class="form-group row <?php echo (!empty($email_err)) ? 'has-error' : ''; ?>">
                                <div class="col-sm-3"><label>Email</label></div>
                                <div class="col-sm-7"><input type="text" class="form-control" id="email" name="email" placeholder="Email Address" value="<?php echo $email; ?>" /></div>
                            ...
                            <input type="submit" class="btn btn-primary" value="Add"/>&nbsp;<input type="reset" class="btn btn-outline-primary" value="Clear"/>&nbsp;<a href="ViewEmployees.php"><input type="cancel" class="btn btn-outline-secondary" value="Cancel"/></a>
